...
class IndexingJobManager:
    def __init__(self):
        ...
        self.table_client = AzureClientManager.initialize_table_client(
            IndexingJobSettings.INDEXING_TABLE_NAME
        )
    ...
    def check_ingestion_job_status(self, job_id: str) -> Dict[str, Any]:
        ...
        return (
            {"status": status, "message": f"Indexing job {status}"}
            if status != IndexingJobSettings.ERROR_STATUS
            else {"status": IndexingJobSettings.ERROR_STATUS, "message": "Error checking job status"}
        )
    ...
    def check_job_status(self, job_id: str) -> Dict[str, Any]:
        ...
        if (
            table_status == IndexingJobSettings.COMPLETED_STATUS
            and api_status == IndexingJobSettings.COMPLETED_STATUS
        ):
            return {"status": IndexingJobSettings.COMPLETED_STATUS, "message": "Job completed successfully"}
        elif (
            table_status == IndexingJobSettings.FAILED_STATUS
            or api_status == IndexingJobSettings.FAILED_STATUS
        ):
    ...
    def update_job_status(self, job_id: str, status: str):
        self.table_client.update_entity(
            {"PartitionKey": "indexing", "RowKey": job_id, "status": status}
        )
    ...
    async def process_indexing_job(self, job_info: Dict[str, Any]):
        ...
        try:
            ...
            while True:
                ...
                if status["status"] in [
                    IndexingJobSettings.COMPLETED_STATUS,
                    IndexingJobSettings.FAILED_STATUS,
                ]:
                    break
            ...
            logger.info(
                f"{'Completed' if status['status'] == IndexingJobSettings.COMPLETED_STATUS else 'Failed'} "
                f"indexing job for container: {container_name}"
            )
...
def _get_job_manager() -> IndexingJobManager:
    global job_manager
...
def create_ingestion_job(container_name: str) -> Dict[str, Any]:
    return _get_job_manager().create_ingestion_job(container_name)


def check_ingestion_job_status(job_id: str) -> Dict[str, Any]:
    return _get_job_manager().check_ingestion_job_status(job_id)


def check_job_status(job_id: str) -> Dict[str, Any]:
    return _get_job_manager().check_job_status(job_id)


def delete_ingestion_index(job_id: str) -> Dict[str, Any]:
    return _get_job_manager().delete_ingestion_index(job_id)


def update_job_status(job_id: str, status: str):
    _get_job_manager().update_job_status(job_id, status)


async def process_indexing_job(job_info: Dict[str, Any]):
    await _get_job_manager().process_indexing_job(job_info)
